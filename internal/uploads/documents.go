...
func StoreDocument(contents []byte, uploadDirectory string, encryptionKeyring Keyring) (StoredDocument, bool, error) {
	contentType, extension, detected := detectDocumentType(contents)
	if !detected {
		return StoredDocument{}, false, nil // fmt.Errorf("cannot detect document type")
	}
	...
	uuid, err := identifiers.NewUUID()
	if nil != err {
		return StoredDocument{}, false, fmt.Errorf("cannot generate UUID, %w", err)
	}
	storagePath := filepath.Join(uploadDirectory, uuid) + extension
	if encrypted {
		storagePath += ".enc"
	}
