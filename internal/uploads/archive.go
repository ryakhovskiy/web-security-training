...
func ExtractTaxDocumentArchive(encryptionKeyring Keyring, contents []byte, extractionDirectory string) (ExtractedTaxDocumentArchive, error) {
	...
	for _, entry := range archiveReader.File {
		...
		if entry.FileInfo().Mode()&fs.ModeSymlink != 0 {
			return ExtractedTaxDocumentArchive{}, fmt.Errorf("archive entry contains symbolic links")
		}
		if strings.ContainsAny(entry.Name, "\\") {
			return ExtractedTaxDocumentArchive{}, fmt.Errorf("archive entry contains backslashes")
		}
		if filepath.IsAbs(entry.Name) {
			return ExtractedTaxDocumentArchive{}, fmt.Errorf("archive entry contains absolute paths")
		}
	...
	for _, entry := range archiveReader.File {
		...
		if !isInsideDirectory(importDirectory, entryDestination) {
			return ExtractedTaxDocumentArchive{}, fmt.Errorf("archive entry escapes the extraction directory")
		}
		...
		contentType, _, detected := detectDocumentType(entryContents)
		if !detected {
			return ExtractedTaxDocumentArchive{}, fmt.Errorf("Unsupported document type in the archive")
		}
		if err != nil {
			return ExtractedTaxDocumentArchive{}, &ArchiveImportError{Message: "Choose a valid ZIP archive.", StatusCode: 400}
		}
		contentTypeMime := mime.TypeByExtension(filepath.Ext(entry.Name))
		if contentType == "" {
			if contentTypeMime != "" {
