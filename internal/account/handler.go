...
func (handler *Handler) UpdateEmail(responseWriter http.ResponseWriter, request *http.Request) {
	...
	if nil != err {
		handler.errorPage(responseWriter, http.StatusForbidden, "Invalid Request", "The submitted form is invalid. Re-enter your current password to change your email")
		return
	}
	correct := passwords.Verify(password, current.User.PasswordHash)
	if !correct {
		handler.errorPage(responseWriter, http.StatusForbidden, "Invalid Request", "The submitted current password is invalid. Re-enter your current password to change your email.")
		return
	}
